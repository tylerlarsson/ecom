          ...
          <div className="dropzone__imgs-wrapper">
            {files.map((file, i) => (
              <div className="dropzone__img" key={i} style={{ backgroundImage: `url(${file.preview})` }}>
                ...
                <button type="button" className="dropzone__img-delete" onClick={e => this.removeFile(i, e)}>
                  Remove
                </button>
              </div>
            ))}
...
const renderDropZoneMultipleField = props => <DropZoneMultipleField {...props.input} />;
